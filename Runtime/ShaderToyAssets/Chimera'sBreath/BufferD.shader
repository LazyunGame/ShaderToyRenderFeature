Shader "ShaderToy/Chimera's Breath/BufferD"
{
    Properties
    {
        iChannel0 ("Texture 0", 2D) = "white" {}
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            vec3 getPalette(float x, vec3 c1, vec3 c2, vec3 p1, vec3 p2)
            {
                ...
                return clamp(mix(mul(m, pws), mul(m2, pws), step(x2, 0.5)), 0., 1.);
            }

            vec4 pal(float x)
            {
                vec3 pal = getPalette(-x, vec3(0.2, 0.5, .7), vec3(.9, 0.4, 0.1), vec3(1., 1.2, .5),
                                      vec3(1., -0.4, -.0));
                return vec4(pal, 1.);
            }
            ...
            vec4 pal2(float x)
            {
                vec3 pal = getPalette(-x, vec3(0.4, 0.3, .5), vec3(.9, 0.75, 0.4), vec3(.1, .8, 1.3),
                                      vec3(1.25, -0.1, .1));
                return vec4(pal, 1.);
            }
            ...
            float4 frag(v2f i) : SV_Target
            {
                ...
                col += .0025 / (0.0005 + pow(length(uv - point2(iTime)), 1.75)) * dt * 0.12 *
                    pal2(iTime * 0.05 + 0.675);
                #endif
