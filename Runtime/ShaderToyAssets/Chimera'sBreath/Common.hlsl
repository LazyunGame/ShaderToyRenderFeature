...
vec4 solveFluid(sampler2D smp, vec2 uv, vec2 w, float time, vec3 mouse, vec3 lastMouse)
{
    ...
    vec2 viscForce = vec2(v,v)*laplacian;
    ...
    data = clamp(data, vec4(vec2(-10,-10), 0.5 , -10.), vec4(vec2(10,10), 3.0 , 10.));
